...
The program gives seven commands and one skill. `init` writes the files of a project into the current directory, then starts the coding agent with the skill `init`. `list` names each skill. `check` reads each path and each name in code font in `AGENTS.md`, in `.hod/PROJECT.md` and in each rule, and reports one that the project does not hold. `update` installs the newest build over this one and writes the project files again, `update --project` writes those files alone, `update --check` reports each change without a write, and `update --force` writes over a file that the program does not own. `worktree:create [branch]` adds a linked worktree with that branch at `$HOME/.hod/worktrees/<project>/<branch>`, where `<project>` is the name of the directory of the main checkout, thus git in the main checkout sees no worktree and the user commits no line for it; without a branch it names the branch with one word from `FIRST` and one word from `SECOND` in `src/worktree.rs`, such as `quiet-harbor`, that no branch and no directory holds. `worktree:merge [message]` merges the branch of the linked worktree that holds the current directory into the branch of the main checkout with `git merge --no-ff`, then removes the worktree and the branch; without a message it asks for one, and `worktree:merge --keep` stops after the merge. The two commands use git alone, thus `worktree:merge` closes a worktree that any tool wrote. Each one writes `cd <path>` as its last line, because the shell of the user stands in the directory that `worktree:merge` removed, and `worktree:create` gives a directory that the user enters. A colon groups two commands, and a hyphen groups two skills, because a slash command holds no colon (refer to section 1). `completions` writes a completion script for a shell. A first argument that is not one of these seven names is a skill, thus a skill cannot take the name of a command. The skill `init` is the one exception, because the command `init` starts it, and a test in `src/skills.rs` reads the two rules.
