...
description: "The rules for the state of an object. Use when you write a new class, a new struct or a new record, when you write a constructor, when you add a property to a type that exists, when you write a setter or a method that writes a property of its own object, when a caller needs an object with one different value, when a method sorts, adds to or removes from a collection that it received, when a method gives back a collection that a property holds, when you fill a property in a step after the construction, when you decide that a different developer can extend a class of this project, and when the code carries `set`, `mut`, `var`, `open`, `extends`, `clone` or `with`."
---
...
| --- | --- | --- | --- |
| PHP | `final class` | `public readonly` | `new self(...)` |
